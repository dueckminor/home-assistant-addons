...
SERVICES = [
    ...
    {
        "name": "security",
        "path": WORKSPACE_ROOT / "web" / "security",
        "command": ["./node_modules/.bin/vite", "serve"],
        "port": 3002,
    },
    {
        "name": "mqtt-bridge",
        "path": WORKSPACE_ROOT / "web" / "mqtt-bridge",
        "command": ["./node_modules/.bin/vite", "serve"],
        "port": 3003,
    },
    {
        "name": "alphaess",
        "path": WORKSPACE_ROOT / "web" / "alphaess",
        "command": ["./node_modules/.bin/vite", "serve"],
        "port": 3004,
    },
]
...
def check_and_install_dependencies(service):
    """Check if npm dependencies are installed and install them if needed"""
    ...
    if not package_json.exists():
        raise Exception(f"package.json not found in {service_path}")

    # Check if node_modules exists and has content
    needs_install = False
    if not node_modules.exists():
        needs_install = True
        print(f"⚠ node_modules not found for {service['name']}")
    else:
        # Check if vite binary exists (main dependency we need)
        vite_bin = node_modules / ".bin" / "vite"
        if not vite_bin.exists():
            needs_install = True
            print(f"⚠ vite binary not found for {service['name']}")

    if needs_install:
        print(f"📦 Installing npm dependencies for {service['name']}...")
        try:
...
def start_service(service):
    ...
    try:
        check_and_install_dependencies(service)
    ...
    if process.poll() is None:
        print(
            f"✓ {service['name']} started (PID: {process.pid}) on port {service['port']}"
        )
...
def stop_service(service_name, pid):
    ...
    try:
        ...
        print(f"✓ {service_name} stopped (PID: {pid})")
    except Exception as e:
        print(f"⚠ Could not stop {service_name} (PID: {pid}): {e}")
...
def start_servers():
    ...
    for service_name in list(pids.keys()):
        if not is_process_running(pids[service_name]):
            print(f"⚠ {service_name} (PID: {pids[service_name]}) is no longer running")
    ...
    for service in SERVICES:
        ...
        if service_name in pids and is_process_running(pids[service_name]):
            print(
                f"✓ {service_name} already running (PID: {pids[service_name]}) on port {service['port']}"
            )
        else:
            try:
...
def stop_servers():
    ...
    for service_name, pid in pids.items():
        if is_process_running(pid):
            ...
            print(f"⚠ {service_name} (PID: {pid}) was already stopped")

    # Clean up PID file
    if PID_FILE.exists():
        PID_FILE.unlink()

    print("🛑 All development servers stopped.")
...
def show_status():
    ...
    for service in SERVICES:
        ...
        if service_name in pids:
            ...
            if is_process_running(pid):
                print(
                    f"  ✓ {service_name}: running (PID: {pid}) on port {service['port']}"
                )
            else:
                print(f"  ✗ {service_name}: stopped (stale PID: {pid})")
        else:
